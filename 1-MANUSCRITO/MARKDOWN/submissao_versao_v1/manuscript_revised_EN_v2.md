...
When viewed through the lens of aggregates and organometallic associations, the role of Fe and Al as a bridge between microbially processed organic matter and mineral surfaces provides a plausible mechanism for the persistence of P associated with organomineral matrices in highly weathered soils [@Wagai2020].
...
The fitted GLM indicated a statistically significant land-use effect on FSNSI (p < 0.001) and good model fit (Deviance/df = 0.082; Pearson/df = 0.075). Adjusted marginal estimates indicated higher functionality in the Eucalyptus system, whose mean differed significantly from native Cerrado (p < 0.05). Adjusted marginal means ranked in decreasing order as Eucalyptus (Exp(B) = 6.07; 95% CI: 5.45-6.69), Mahogany (Exp(B) = 4.54; 95% CI: 3.98-5.10), Cerrado (Exp(B) = 4.42; 95% CI: 3.80-5.04), Agriculture (Exp(B) = 4.25; 95% CI: 3.68-4.82), and Teak (Exp(B) = 2.77; 95% CI: 2.35-3.19), confirming that functionality was highest under continuous organic inputs without post-establishment soil disturbance and lowest where biochemical impedance restricted cycling.

Eucalyptus exceeded Cerrado (Exp(B) = 6.07 versus 4.42), but this pattern is conditional on the sampling window and on how the index weights chemical capital (N and P) and physical constraint (bulk density). The result is consistent with higher inputs and limited post-establishment disturbance [@Iversen2022], and evidence indicates that well-managed plantations may surpass native baselines for specific soil functions under optimized inputs [@Tang2023]. Nevertheless, the confidence intervals of Mahogany (3.98–5.10), Cerrado (3.80–5.04), and Agriculture (3.68–4.82) overlap substantially, indicating that these three systems are statistically indistinguishable in FSNSI at the current sample size. Only Eucalyptus and Teak occupy non-overlapping ranges, supporting unambiguous separation exclusively at the extremes of the functionality gradient. These overlapping intervals underscore that the ranking should be interpreted as a tendency rather than a categorical hierarchy and that management inferences drawn from intermediate positions require additional replication or complementary indicators to achieve confirmatory power.
...
The present study focused on nitrogen and phosphorus dynamics in humic and labile fractions; soil organic carbon (SOC) stocks were not determined. Because SOM is the primary matrix from which humic substances are derived, future studies should integrate SOC measurements to evaluate carbon-to-nutrient stoichiometry and to provide a more complete picture of organomineral stabilization under different land uses. Despite this limitation, the N and P fractionation approach, combined with structural modeling (PLSR and PLS-SEM) and fuzzy integration, captures the dominant nutrient-retention mechanisms and yields an operational sustainability indicator consistent with independent evidence from tropical Oxisols [@Gerke2022; @Lavallee2020].
...
This study confirms that soil nutritional functionality in Cerrado Oxisols is primarily regulated by humic retention mechanisms, with bulk density acting as a key constraint modulating biogeochemical fluxes. The proposed Fuzzy Soil Nutritional Sustainability Index (FSNSI) successfully discriminated land-use systems, providing an operational indicator that integrates chemical capital and physical constraint into a single metric for environmental monitoring and assessment. FSNSI enables management prioritization by identifying systems with compromised functionality, such as conventional agriculture, and those with higher functional performance, such as forest plantations that maintain continuous litter input without post-establishment soil disturbance. However, the overlapping confidence intervals among intermediate-ranked systems (Mahogany, Cerrado, and Agriculture) indicate that categorical separation at the mid-range requires additional replication or complementary indicators, including soil organic carbon dynamics.

From a practical standpoint, the results support the following management recommendations for Cerrado Oxisols: (i) silvicultural systems with permanent canopy cover (e.g., Eucalyptus, *Khaya ivorensis*) should be prioritized on degraded lands because they preserve humic stabilization pathways and maintain low bulk density; (ii) conventional agriculture should incorporate conservation practices such as no-till, cover cropping, and organic amendments to mitigate the observed reduction in humic efficiency and increased residual fractions; (iii) Teak plantations require litter-management strategies (e.g., mulching, intercropping with fast-decomposing species) to overcome the biochemical impedance caused by recalcitrant leaf litter and soil compaction; and (iv) FSNSI, combined with spatial monitoring layers, can serve as a routine screening tool for environmental agencies to identify functional-risk hotspots and prioritize intervention areas in land-management programs.
